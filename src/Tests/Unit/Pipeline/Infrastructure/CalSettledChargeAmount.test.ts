...
/**
 * Map a Cal row as a card issuer and return the charged amount.
 *
...
describe('settled Cal charge amount', () => {
  it('reads the charged amount, not the deal total, on an instalment payment', () => {
    ...
    const instalmentPayment = amountOf({ ...SETTLED, trnAmt: 1615, amtBeforeConvAndIndex: 161.5 });
    expect(instalmentPayment).toBe(-161.5);
  });

  it('reads the post-adjustment amount on an index-linked charge', () => {
    const indexLinked = amountOf({ ...SETTLED, trnAmt: 760, amtBeforeConvAndIndex: 800 });
    expect(indexLinked).toBe(-800);
  });

  it('records a fee billed elsewhere as zero rather than inventing a charge', () => {
    const feeBilledElsewhere = amountOf({ ...SETTLED, trnAmt: 22.29, amtBeforeConvAndIndex: 0 });
    expect(feeBilledElsewhere).toBe(0);
  });
  ...
  it('falls back to the deal total on a pending row, which has no settled amount', () => {
    ...
    const pending = amountOf({ date: '2026-02-25', description: 'MERCHANT', trnAmt: 500 });
  ...
  it('is unchanged for the ordinary case where the two agree', () => {
    ...
    const bothAgree = amountOf({ ...SETTLED, trnAmt: 51.77, amtBeforeConvAndIndex: 51.77 });
    expect(bothAgree).toBe(-51.77);
  });

  it('does not disturb an institution that sends neither field', () => {
    const neitherField = amountOf({ date: '2026-02-25', description: 'MERCHANT', amount: 42 });
    expect(neitherField).toBe(-42);
  });
});
