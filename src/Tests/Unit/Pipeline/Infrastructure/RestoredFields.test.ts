 ...
 *
 * Every institution routes through `autoMapTransaction`, which builds the
 * transaction from WK alias hits alone — so five optional `ITransaction`
 * fields were never populated even when the payload carried them. These tests
...
describe('restoreProviderFields', () => {
  ...
  it('maps the instalment transaction type', () => {
    expect(mapped({ numOfPayments: 10, curPaymentNum: 3 }).type).toBe(TransactionTypes.Installments);
  });
  ...
  it('honours an explicit non-instalment transaction-type code', () => {
    ...
    expect(mapped({ trnTypeCode: '5', debCrdDate: '2026-03-02' }).type).toBe(TransactionTypes.Normal);
