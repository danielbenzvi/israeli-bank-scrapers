/**
 * Amex — charges posting outside the current statement cycle are rows,
 * not summary.
 *
 * `mergeAmexRows` read two containers and dropped a third:
 ...
 *
 * A replay could not have caught this one: stored Amex rows came from the same
 * lossy path, so a loss that was always present looks like agreement. It took
 * reading the raw responses. Its Isracard sibling only gave itself away because
 * that account's stored rows predated the migration.
 *
...
describe('Amex/outOfStatementChargeDateVouchers', () => {
  ...
  it('merges all three containers into one list', () => {
    ...
    const rows = mergeAmexRows(body) as readonly Record<string, unknown>[];
    const seqs = rows
  ...
  it('keeps returning nothing for an empty response', () => {
    const rows = mergeAmexRows({ data: null });
    expect(rows.length).toBe(0);
  });
});
