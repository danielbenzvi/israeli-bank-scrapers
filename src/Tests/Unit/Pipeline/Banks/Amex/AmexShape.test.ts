...
describe('AmexShape helpers', () => {
  ...
  it('extractCards coerces a numeric companyCode to a string', () => {
    const body = cardListBody([{ cardSuffix: '2222', companyCode: 11 }]);
    const args = accountsArgs(body);
    const accounts = extractCards(args);
    expect(accounts[0]).toEqual({ cardSuffix: '2222', companyCode: '11' });
  });

  it('extractCards tolerates an already-parsed array (defensive)', () => {
    const body = { data: { cardsList: [{ cardSuffix: '3333', companyCode: '11' }] } };
    const args = accountsArgs(body);
    const accounts = extractCards(args);
    expect(accounts).toEqual([{ cardSuffix: '3333', companyCode: '11' }]);
  });
