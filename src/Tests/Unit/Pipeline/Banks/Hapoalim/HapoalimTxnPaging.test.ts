 ...
 */
function argsFor(body: object, startDate = '2026-02-20'): IExtractPageArgs<IHapoalimAcct, HapoalimCursor> {
  return {
    body,
    cursor: false,
    acct: ACCT,
...
describe('Hapoalim/transactions paging', () => {
  it('asks for another window when the bank filled the page at its own cap', () => {
    ...
    const body = { numItemsPerPage: 3, transactions: [row(20260317), row(20260401), row(20260820)] };
    const page = txnsExtractPage(argsFor(body));
  ...
  it('stops on a short page — the window was fully served', () => {
    ...
    expect(txnsExtractPage(argsFor(body)).nextCursor).toBe(false);
  });
  ...
  it('stops once the next window would start before the caller asked', () => {
    ...
    expect(txnsExtractPage(argsFor(body, '2026-02-20')).nextCursor).toBe(false);
  });
  ...
  it('returns no rows, and no cursor, for an empty response', () => {
    const page = txnsExtractPage(argsFor({}));
...
describe('Hapoalim/transactions URL', () => {
  it('ends the first window today', () => {
    const url = String(txnsUrl(ACCT, false, ctxFrom('2026-02-20') as never));
  ...
  it('ends a later window where the cursor says, keeping the same start', () => {
    const url = String(txnsUrl(ACCT, '20260316', ctxFrom('2026-02-20') as never));
